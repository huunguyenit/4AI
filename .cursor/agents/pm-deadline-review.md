...
5. **Cổng PM — chỉ với UR ở `DD`.** Đọc TLKS rồi quyết định:
   ...
   - `KL` — vượt khả năng FBO. **Bắt buộc có `PMReview` nối `BASED_ON` tới một
     `CapabilityVerdict` verdict `khong`** (xem [pm-capability-graph](../rules/pm-capability-graph.mdc)). Không có thì hỏi PM,
     không tự kết luận.
...
7. **Yêu cầu nhắc tạo bảng / thêm cột** → xem [fbo-new-table-proposal](../rules/fbo-new-table-proposal.mdc). Gợi ý đính vào UR
...
- `ghiChuDdl` PHẢI là script SQL thật (xem [fbo-new-table-proposal](../rules/fbo-new-table-proposal.mdc)) — báo cáo tô màu cú pháp
