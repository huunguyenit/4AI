...
- [ ] B2. XML: code/order + isPrimaryKey ([xml-structure.md](references/xml-structure.md))
- [ ] B3. Validation trong commands ([check-trung.md](references/check-trung.md))
- [ ] B4. Bảng: cấp đặc tả cho bộ sinh, KHÔNG tự chạy DDL ([database.md](references/database.md))
- [ ] B5. Verify MCP
```
...
[xml-structure.md](references/xml-structure.md)
...
[check-trung.md](references/check-trung.md)
...
| File | Nội dung |
|------|----------|
| [xml-structure.md](references/xml-structure.md) | Dir/Grid, khóa, field types, khối `<script>` |
| [check-trung.md](references/check-trung.md) | 4 pattern validation + template SQL nguyên văn từ sản phẩm |
| [database.md](references/database.md) | Đặc tả bảng, cột audit, structure file, verify |
