...
// Bất biến chữ ký: xem guard trong runSync. `GENERATED_MARK` phải khớp chuỗi mà
// `emit/common.mjs` đặt vào banner — đổi một nơi thì đổi cả hai.
const GENERATED_MARK = 'GENERATED BY 4AI';
const NO_COMMENT_EXT = /\.(json|jsonl)$/i;
...
export async function runSync(opts) {
  ...
  // `{{HUB}}` được giải trong vòng emit bên dưới (resolveMcpServers), không phải ở đây.
  ...
  for (const target of targets) {
    ...
    for (const tool of tools) {
      ...
      const mcpServers = resolveMcpServers(thoServers);
    ...
    // Mọi file 4AI sinh ra PHẢI mang chữ ký — không có nó, người mở file không biết mình đang
    // sửa một bản chép sẽ bị ghi đè ở lần sync sau. Chỉ miễn cho định dạng không mang được
    // comment (JSON/JSONL); chữ ký của chúng nằm ở `.4ai/manifest.json` (path + sha256).
    const unsigned = textFiles.filter(
      (f) => !NO_COMMENT_EXT.test(f.relPath) && !String(f.content).includes(GENERATED_MARK));
    if (unsigned.length > 0) {
      process.stderr.write(
        `4ai: BUG — ${unsigned.length} file emit cho target \`${target.name}\` thiếu chữ ký `
        + `"${GENERATED_MARK}":\n`);
      for (const f of unsigned.slice(0, 10)) process.stderr.write(`  ${f.relPath}\n`);
      if (unsigned.length > 10) process.stderr.write(`  … và ${unsigned.length - 10} file nữa\n`);
      return 1;
    }
