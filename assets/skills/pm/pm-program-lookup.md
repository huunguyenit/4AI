...
Program path là căn cước của mọi thay đổi (rule `erp-program-scope`). Nguồn sự thật
là bảng `nbdmda` trong DB QLDA nội bộ (DB nghiệp vụ QLDA) — hệ thống quản lý dự án dùng chung của cả
...
- `list_programs { query: "<tên khách hoặc mã hoặc đoạn đường dẫn>" }` — khớp vào
  ...
  path nhưng chưa biết mã — xem `pm-program-detection`).
...
- **`sp` là mã ghép, không phải một chuỗi liền** — `FBOR2SP2422` = dòng `FBO` + vòng thiết kế
  `R2` + `SP` + số hiệu `2422` (đọc là **2.4.2.2**). Mỗi chữ số là một thành phần, đệm `0` cho
  đủ bốn: `SP24` = 2.4.0.0 nên **mới hơn** `SP224` = 2.2.4.0. Cách đọc đầy đủ, ba dạng ngoại
  lệ chưa chốt, và bẫy "`ma_pbsp` lệch với đường dẫn program": `erp-glossary-reference` →
  `abbreviations.md`, mục *Đọc mã phiên bản sản phẩm*.
...
tiếp tục — báo rõ không tìm thấy, hỏi người dùng theo `pm-scope-clarification`. Không có
đường ghi vào `nbdmda` từ hub này (chỉ đọc — xem `erp-sql-access`).

## Bẫy

- Mã dự án thật lệch khỏi tên khách rút gọn theo cảm tính rất phổ biến: mã hay có hậu tố
  chi nhánh (`<TEN><CN>`) chứ không phải tên trần; và có khách mà mã trực giác lại trúng một
  dự án dòng cũ **khác hẳn**, bản mới mang hậu tố sản phẩm (`<TEN>_FBO`). Luôn tra
