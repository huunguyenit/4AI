...
- [ ] 10. Verify tồn tại + size/date; ghi ledger + checklist bàn giao (PH thử, menu HDDT)
```
...
6. **`EIReleasedInvoice*` không thuộc gói** — đó là Hóa đơn bán ra, cần UR riêng. Đừng nhầm với `EIReleaseType` (kiểu phát hành, thuộc gói).
7. **Menu `wcommand` là một phần của gói**: dòng 11.12.20–11.12.64 còn thiếu phải INSERT (trừ ba dòng `EIRelease…`), trình câu lệnh cho user duyệt. Ngoài phạm vi đó, **không** deploy SQL / `dmhddtbs` trong skill này — chuyển `erp-einvoice-customize`.
8. **Không** sửa file bằng PowerShell `Set-Content` cho XML/JS cần accept trong IDE — file binary/runtime copy thì dùng lệnh copy sau khi đã duyệt.
9. Ghi ledger theo `pm-ledger-maintain`; bàn giao theo `pm-handover-author` (verify: mở menu HDDT, phát hành thử nếu môi trường cho phép).
