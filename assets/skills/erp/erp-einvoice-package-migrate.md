...
6. **Chép file controller chưa phải là xong**: DOCTYPE của nó trỏ sang `..\Include\…` mà dự án cũ thường chưa có. Chép xong phải `index_program` **lại** (index là ảnh chụp), rồi chạy `resolve_entities` trên từng file đã chép, mang qua mọi entity có `systemPath` mà `exists: false`, rồi **lặp lại** vì `.ent` tự khai SYSTEM tiếp. Entity `systemPath: null` là khai inline — bỏ qua, không phải file thiếu.
