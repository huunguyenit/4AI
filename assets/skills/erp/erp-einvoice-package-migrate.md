...
6. **Chép file controller chưa phải là xong**: DOCTYPE của nó trỏ sang `..\Include\…` mà dự án cũ thường chưa có. Chép xong phải `index_program` **lại** (index là ảnh chụp), rồi chạy `resolve_entities` trên từng file đã chép, mang qua mọi entity có `systemPath` mà `exists: false`, rồi **lặp lại** vì `.ent` tự khai SYSTEM tiếp — quét cả `.ent` đã có sẵn ở đích, không chỉ cái vừa chép. Closure phình lên hàng trăm file là dấu hiệu vượt bản: dừng, đừng chép tiếp. Entity `systemPath: null` là khai inline — bỏ qua, không phải file thiếu.
