...
kind: skill
domain: erp
description: Setup pivot báo cáo FBO — RS1 xSearch/xPivot/nHeader + dataFields, RS2 #xPivot CROSS JOIN #nPivot, Grid pivot, ẩn cột theo DetailBy. Kế thừa fbo-report. Mở khi xoay 12 tháng, kế hoạch/thực tế.
requires: [4ai-fbo]
disable-model-invocation: true
see-also: [fbo-report, erp-sql-style]
version: 1
---
**Kế thừa `fbo-report`** — đọc skill đó trước (Filter `field@name`, proc param tiếng Anh, sign, format, Key). Skill này chỉ **setup xoay cột**.

Load DATA và bốn vùng của proc báo cáo: `fbo-report`.
...
- User nói fbo-report-pivot sau khi đã có proc nền

## Workflow

```
- [ ] 1. Xong fbo-report (Filter + proc param + Key + DATA)
...
```
❌  Viết lại format/Key/param trong skill này — thuộc fbo-report
