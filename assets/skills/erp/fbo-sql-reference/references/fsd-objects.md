...
`fsd_StringToTable` trả cột `val` — luôn `rtrim(val)` khi so sánh (xem `fbo-create-category`).
...
`fsd_addFields` là đường được duyệt để thêm cột — xem `fbo-new-table-proposal` trước khi tự
viết `ALTER TABLE`.
