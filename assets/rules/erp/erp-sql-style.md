...
see-also: [erp-sql-reference, erp-report-create, erp-sql-access, erp-sql-schema-check]
version: 4
---
...
- Danh sách tham số: **mỗi tham số một dòng**, không dồn ngang. Chữ ký là thứ người khác đọc
  đầu tiên để biết gọi proc thế nào.
...
### 2.4. Độ dài dòng, `CASE WHEN`, dynamic SQL

**Trần ~120 ký tự một dòng.** Dài hơn thì xuống dòng theo nhóm cột, không kéo ngang. Luật
này cộng dồn với luật "`SELECT` wrap khi > 8 cột": chạm cái nào trước thì xuống dòng ở đó.

**`CASE WHEN` dài — mỗi nhánh một dòng**, `WHEN` thụt sâu hơn `CASE` một tab:

```sql
SELECT a.ma_kh
	 , CASE WHEN a.loai_ct = '1' THEN a.tien_nt
	        WHEN a.loai_ct = '2' THEN -a.tien_nt
	        ELSE 0
	   END AS tien_quy_doi
	FROM r70$000000 a
```

`CASE WHEN` ngắn (một điều kiện, vế ngắn) để **cùng dòng** — tách dòng ở đây chỉ làm loãng.

**Dynamic SQL format y như SQL thường.** Chuỗi trong `SET @q = '…'` vẫn là câu lệnh người
khác phải đọc và debug — **KHÔNG ĐƯỢC** dồn cả proc vào một chuỗi dài một dòng:

```sql
SET @q = 'SELECT a.ma_kh, a.ma_vt, SUM(a.so_luong) AS so_luong'
	+ ' INTO #agg'
	+ ' FROM r70$%Partition a WITH(NOLOCK)'
	+ ' WHERE ' + @Key
	+ ' GROUP BY a.ma_kh, a.ma_vt'
```

Giữ khoảng trắng đầu mỗi mảnh nối — thiếu một dấu cách là câu lệnh dính chữ và lỗi lúc chạy,
không lỗi lúc biên dịch.
...
## 11. Sửa SQL đã có — format KHÔNG được đổi nghiệp vụ

Sửa format và sửa logic là **hai việc**, và luật này tồn tại vì chúng hay bị trộn vào một
diff. Khi đã trộn thì không ai soi được nữa: 400 dòng đổi, trong đó một dòng đổi điều kiện
lọc, và nó lọt.

**KHÔNG ĐƯỢC** đổi trong lượt reformat:

- alias bảng (`a` → `x`), kể cả khi thứ tự alphabet đang sai
- thứ tự và tên tham số của proc — đổi là **breaking change** với mọi Filter đang gọi
- kiểu và độ dài dữ liệu của tham số / cột
- điều kiện lọc: `@Key`, điều kiện quyền đơn vị, điều kiện trạng thái (`status`, `ma_gd`)

Muốn đổi thật thì tách ra một lượt riêng và nói rõ đổi cái gì, vì sao.

**Comment:** chỉ viết khi giải thích một logic khó. Không chú thích lại thứ câu lệnh đã nói.

### 11.1. Thêm một trường — bốn chỗ, không phải một

Trường mới chỉ thêm ở đầu chuỗi thì nó biến mất ở giữa đường, và triệu chứng là cột rỗng
chứ không phải lỗi. **BẮT BUỘC** rà đủ:

- [ ] struct `SELECT TOP 0 … INTO #temp` — có cột mới chưa
- [ ] mọi `INSERT #temp SELECT …` — đủ số cột và **đúng thứ tự** chưa
- [ ] `SELECT` output cuối proc — có trả cột mới ra không
- [ ] search / paging — cột mới có cần vào điều kiện tìm kiếm hay `ORDER BY` không

`INSERT #temp SELECT …` không liệt kê cột đích (rule §7.3), nên **thứ tự** là thứ duy nhất
giữ đúng ánh xạ. Thêm cột vào giữa struct mà không sửa mọi `INSERT` là đổi nghĩa toàn bộ
các cột phía sau, im lặng.

**Chỉ thêm trường khi người dùng yêu cầu.** Thấy bảng có sẵn cột hay hay không phải là lý do
để đưa nó vào output.

---



## 12. Checklist nhanh trước khi giao SQL

- [ ] Param / biến ENGLISH; Sign `--C--NGUYENTDH`; có `@UserID` `@Admin` `@Language`
- [ ] Keyword HOA; alias `a`→`z`; tab đúng level; `SELECT` wrap khi > 8 cột
- [ ] Dòng ≤ ~120 ký tự; `CASE WHEN` dài tách từng nhánh; dynamic SQL format nhiều dòng
- [ ] Mọi bảng nguồn/đích đã soi bằng `query_sql { object }` — không cột nào là phỏng đoán
- [ ] Reformat KHÔNG đổi alias / thứ tự tham số / kiểu dữ liệu / điều kiện lọc
- [ ] Thêm trường: đủ struct `#temp` + mọi `INSERT` + `SELECT` output + search/paging
