...
Lệch giữa số tick trong `nbctdaumuc` và số đếm được từ nội dung UR → nêu trong mục **Cần
làm rõ**, không tự sửa số tick. Tên cột thật của `nbctdaumuc` (đầu mục, cờ check, số
lượng) cần xác nhận qua `query_sql` trước khi viết câu SELECT — chưa xác nhận thì đừng đoán
tên cột.
