...
tools: [Read, Grep, Glob, Write, Bash, mcp__4ai-fbo__list_programs, mcp__4ai-fbo__query_sql, mcp__4ai-fbo__get_review_dataset]
model: inherit
requires: [4ai-fbo]
...
1. **Lấy dữ liệu thô — MỘT lần.** Gọi `get_review_dataset` (không tự viết `query_sql` rời
   ...
   - Không có (rà soát TOÀN BỘ) → `get_review_dataset({ pmName: '{PMName}' })`.
