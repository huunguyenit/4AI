<!-- GENERATED BY 4AI — DO NOT EDIT.
     Gộp từ 12 asset luôn-nạp. Nguồn: assets/ trong hub.
...
| `erp-category-create` | Tạo hoặc sửa danh mục FBO — cặp Dir (form) + Grid (list), khai khóa chính, validation trùng và lồng bằng fsd_StringToTable với OldValue, tạo bảng và PK. Mở khi user đưa cặp Dir/Grid. |
...
| `erp-view-design` | Layout form FBO trong Dir/Filter — view@height, view@anchor, view@split, item value (cột px + pattern 1/0/- + token [field]). Mở khi sửa views, merge cột, neo bảng, hoặc hỏi về layout. |
