...
import { isAlwaysOn, emitPaths, referenceDir, referenceRef } from '../paths.mjs';
...
/** Banner cho file reference — không có frontmatter nên banner đứng ngay dòng đầu. */
export function bannerReference(asset, name) {
  return `<!-- GENERATED BY 4AI — DO NOT EDIT.
     Source: assets/skills/${asset.domain}/${asset.id}/references/${name}.md (skill v${asset.version})
     Hub: ${HUB} · Regenerate: node tools/4ai.mjs sync -->`;
}

/**
 * Token `{REFDIR}` trong thân skill → đường dẫn tương đối tới thư mục reference của chính
 * dialect đang emit. Viết thân một lần, mỗi dialect trỏ đúng chỗ của nó.
 * Asset không có reference thì token (nếu lỡ có) trả về nguyên trạng, không đoán.
 */
export function applyRefDir(body, asset, tool, opts = {}) {
  if (!asset.references?.length) return body;
  const refdir = referenceRef(asset, tool, opts);
  return refdir ? body.replaceAll('{REFDIR}', refdir) : body;
}

/**
 * Bản asset đã sẵn sàng cho một dialect: gộp override về path (always/kind mà emitter tự đổi)
 * rồi thay token `{REFDIR}` trong thân theo đúng layout của dialect đó. Dùng CHUNG một object
 * cho cả emitPaths lẫn nội dung file để hai bên không bao giờ tính ra hai đường dẫn khác nhau.
 */
export function forEmit(asset, tool, opts = {}, overrides = {}) {
  const base = { ...asset, ...overrides };
  return { ...base, body: applyRefDir(base.body, base, tool, opts) };
}

/** File reference đi kèm một skill, dưới dạng mô tả file cho writer. */
export function referenceFiles(asset, tool, opts = {}) {
  const refs = asset.references ?? [];
  if (refs.length === 0) return [];
  const dir = referenceDir(asset, tool, opts);
  if (!dir) return [];
  return refs.map((r) => ({
    relPath: `${dir}/${r.name}.md`,
    content: [bannerReference(asset, r.name), '', r.body].join('\n'),
    sourceId: `${asset.id}#${r.name}`,
